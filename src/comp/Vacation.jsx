...
export default function Vacation({ vacation, likedVacations, setUpdate }) {
    ...
    const handleChange = async () => {
        if (likedVacations.map(item => item.id).includes(vacation.id)) {
            const res = await fetch(`http://localhost:10000/vacations/unlike/${localStorage.username}/${vacation.id}`, {
                method: "delete",
                credentials: 'include'
            })
            ...
            const res = await fetch(`http://localhost:10000/vacations/like/${localStorage.username}/${vacation.id}`, {
                method: "put",
                credentials: 'include'
            })
            const data = await res.json()
